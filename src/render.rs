            ...
            BSPTree::Leaf(linedefs) => {
                for x in 0..screen_width {
                    for linedef in linedefs {
                        if let Some((draw_start, draw_end, perp_wall_dist)) = Renderer::ray_wall_intersection(player, linedef, x, screen_width, screen_height) {
                            ...
                            let intersection_x = player.pos.0 + perp_wall_dist * (player.dir.0 + player.plane.0 * (2.0 * x as f64 / screen_width as f64 - 1.0));
                            let tex_x = ((intersection_x - linedef.start.x) / wall_length * wall_length).abs().fract() * texture.width as f64;
                            
                            for y in (draw_start as usize).max(0)..(draw_end as usize).min(screen_height) {
                                let index = y * screen_width + x;
                                if perp_wall_dist < zbuffer[index] {
                                    
                                    let tex_y = ((y as f64 - draw_start) / (draw_end - draw_start) * linedef.height).abs().fract() * texture.height as f64;
...
impl Renderer {
    ...
    fn ray_wall_intersection(player: &Player, linedef: &Linedef, x: usize, screen_width: usize, screen_height: usize) -> Option<(f64, f64, f64)> {
        ...
        if t >= 0.0 && u >= 0.0 && u <= 1.0 {
            ...
            let draw_base = height_diff_scaled;  // Align the base of the wall with the floor level directly
            let mut draw_end = draw_base;  // Start from the base
            let mut draw_start = draw_base - wall_height;  // Wall height extends downwards from the base
            ...
            Some((draw_start, draw_end, perp_wall_dist))
        } else {
            None
        }
    }
